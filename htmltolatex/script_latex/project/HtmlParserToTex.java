...
public class HtmlParserToTex {
	...
	static final int varN[] = {200,400,600,800,1000};
	static final int varL[] = {8,16,32,64,128};
	String reportDir;
	...
	public void collateLatex(){
		...
		/*for(Integer num : varN){
			for(Integer lab : varL){
				...
				Elements tablhdr = table.select("th");
			}
		}*/
		Element tablhdr = doc.select("h3").first();
		Element table = doc.select("table").first();
...
//	}
	...
	/*private void addSecColumnHeader(int value, int span, BufferedWriter latex) throws IOException {
		StringBuilder sb = new StringBuilder("{" + 5 + "} {c|} {N =";
		for 
		sb.append("\n");
		latex.write(sb.toString());
		StringBuilder sb2 = new StringBuilder("\\cline{3-"+ String.format ("%d", TOTCOLUMNS+2) + "}");
