...
public class HtmlParserToTex {
			...
			for(Element tr: table.select("tr")){
				if(isHeader){
					...
					for(Element th : tr.select("th")){
						...
						Pattern p = Pattern.compile("(l)\\s*=\\s*(\\d+)");
					...
					for(Element td : tr.select("td")){
						...
						if(m.find()){
							...
							addTreeItem(kValues[index++], ln.n, fname, algoName, mean, sd);
						}else{
							//addTreeItem(kValues[index++], ln.l, ln.n, fname, algoName, "-", "-");
							addTreeItem(kValues[index++], ln.n, fname, algoName, "-", "-");
						}
	...
	public void addTreeItem(int k, int n, String fileName, String algoName, String value, String sdvalue){
		...
		HashNode nNode = algoNode.getOrMakeChild(Integer.toString(n));
		nNode.getOrMakeChild(Integer.toString(k), value);
		nNode.getOrMakeChild(Integer.toString(k), sdvalue);
		...
		try {
			...
			while((line = reader.readLine()) != null){
				...
				//	System.out.println(line);
					...
					HeaderValue v = new HeaderValue(n);
	...
	public void collateLatex(String fileName){
		...
		try {
			...
			openTable("Performance over N,L" , 1, colCount, latex);
			...
			for(HashNode kNode : algoNode0.getChildren()){
				...
				addMultiCol(kNode.getChildCount(), " c |", boldText("L = " + kNode.data), latex);
			}*/
			/*addLineBreak(latex);
			addHorizontalLine(3, colCount + 2, latex);
			...
			addEmptyItem(latex);
			for(HashNode nNode : algoNode0.getChildren()){
				for(HashNode lNode : nNode.getChildren()){
					for(String lValue : lNode.getKeys()){
			...
			for(HashNode fileNode : root.getChildren()){
				...
				for(HashNode algoNode : fileNode.getChildren()){
					...
					for(HashNode nNode : algoNode.getChildren()){
						for(HashNode lNode : nNode.getChildren()){
							//for(HashNode lNode : nNode.getChildren()){
